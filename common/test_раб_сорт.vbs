...
' Сортировка пузырьком по pinName
For i = 0 To k - 2
    For j = 0 To k - 2 - i
        If ArrDeviceIds(j, 1) > ArrDeviceIds(j + 1, 1) Then
            ' Обмен значениями
            temp0 = ArrDeviceIds(j, 0)
            temp1 = ArrDeviceIds(j, 1)
            temp2 = ArrDeviceIds(j, 2)
            
            ArrDeviceIds(j, 0) = ArrDeviceIds(j + 1, 0)
            ArrDeviceIds(j, 1) = ArrDeviceIds(j + 1, 1)
            ArrDeviceIds(j, 2) = ArrDeviceIds(j + 1, 2)
            
            ArrDeviceIds(j + 1, 0) = temp0
            ArrDeviceIds(j + 1, 1) = temp1
            ArrDeviceIds(j + 1, 2) = temp2
        End If
    Next
Next
