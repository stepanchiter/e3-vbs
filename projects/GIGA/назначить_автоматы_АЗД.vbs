...
Sub FindAndLogOOOSymbols()
    ...
    e3App.PutInfo 0, "Найдено " & allSymbolCount & " символов в проекте. Ищем OOO символы с 'ОД D_Proizv3' = '3' или '4'..."
    ...
    If foundOOOCount = 0 Then
        e3App.PutInfo 0, "Не найдено OOO символов со значением атрибута 'ОД D_Proizv3' равным '3' или '4'."
    Else
...
Sub FindAndLogRelatedDevices()
    ...
    If global_foundOOOIds.Count = 0 Then
        e3App.PutInfo 0, "Нет зафиксированных OOO символов (D_Proizv3=3 или 4) для поиска связанных устройств."
        Exit Sub
    End If
...
Sub UpdateQFComponentsBasedOnOOOAttribute()
    ...
    ' Проверяем, были ли найдены OOO символы на предыдущем шаге
    If global_foundOOOIds.Count = 0 Then
        e3App.PutInfo 0, "COMM: Нет зафиксированных OOO символов (D_Proizv3=3 или 4) для обновления компонентов QF."
        Exit Sub
    End If
