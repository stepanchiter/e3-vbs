...
Sub WriteOOOAttributesFromExcel()
    ...
    For i = LBound(oooNamesArray) To UBound(oooNamesArray) ' Используем LBound и UBound для итерации по отсортированному массиву
        ...
        oooTag = Trim(CStr(excelSheet.Cells(rowNum, 4).Value))    ' Столбец D --> ОД E_TAG
        oooType = Trim(CStr(excelSheet.Cells(rowNum, 7).Value))  ' Столбец G --> ОД E_TYPE
        oooPras = Trim(CStr(excelSheet.Cells(rowNum, 10).Value)) ' Столбец J --> ОД E_Pras
        oooPnom = Trim(CStr(excelSheet.Cells(rowNum, 9).Value))  ' Столбец I --> ОД E_Pnom
        oooInom = Trim(CStr(excelSheet.Cells(rowNum, 12).Value)) ' Столбец L --> ОД E_Inom
        oooDProizv3 = Trim(CStr(excelSheet.Cells(rowNum, 18).Value)) ' Столбец R --> ОД D_Proizv3
        oooIras = Trim(CStr(excelSheet.Cells(rowNum, 13).Value))  ' Столбец M --> ОД E_Iras
        oooDProizv2 = Trim(CStr(excelSheet.Cells(rowNum, 17).Value)) ' Столбец Q --> ОД D_Proizv2
