...
' Дата: 08.07.2025
' Описание: Записывает атрибуты аппаратов в ОООc. Скрипт для автоматической записи значения в атрибут "Тех. описание 3"
...
'           в атрибут "ОД D_Proizv2" соответствующего символа OOS.
'*******************************************************************************
...
Function ExtractNumber(ByVal itemName)
    Dim re, matches
    Set re = New RegExp
    ' Ищем число в конце строки после символов (например, OOS1, -QF1, -KM1, -KL1, -V1)
    re.Pattern = "(\d+)$"
    re.Global = False
...
' === Функция === Получение значения атрибута ОД E_Inom из символа OOS
...
Sub FindAllOOSSymbols(ByVal e3AppObj, ByVal jobObj, ByRef OOSSymbols)
    ...
    e3AppObj.PutInfo 0, "=== ПОИСК ВСЕХ СИМВОЛОВ OOS В ПРОЕКТЕ ==="
    ...
    For i = 1 To symbolCount
        ...
        If LCase(Left(symbolName, 3)) = "OOS" Then
            ...
            If symbolNumber > 0 Then
                ...
                e3AppObj.PutInfo 0, "Найден символ OOS: " & symbolName & " (номер: " & symbolNumber & ", ID: " & symbolIds(i) & ")"
            Else
                e3AppObj.PutInfo 0, "Символ OOS найден, но номер не определен: " & symbolName
            End If
        End If
    Next
    
    e3AppObj.PutInfo 0, "Всего найдено символов OOS с номерами: " & OOSSymbols.Count
...
' === Процедура === Запись атрибутов в символ OOS
Sub WriteAttributesTOOSOSymbol(ByVal e3AppObj, ByVal jobObj, ByVal OOSSymbolId, ByVal number, _
    ...
    e3AppObj.PutInfo 0, "=== ЗАПИСЬ АТРИБУТОВ В СИМВОЛ OOS" & number & " ==="
...
' === Основная процедура === Обработка всех символов OOS и устройств
Sub ProcessAllOOSSymbolsAndDevices()
    Dim e3App, job ' Создаем объекты один раз
    Dim OOSSymbols, qfDevices, kmDevices, klDevices, vDevices ' Добавлен vDevices
    ...
    Dim key
    Dim OOSEInomValue ' Переменная для значения ОД E_Inom
    ...
    e3App.PutInfo 0, "=== СТАРТ ОБРАБОТКИ ВСЕХ OOS СИМВОЛОВ И УСТРОЙСТВ ==="
    
    ' Находим все символы OOS и устройства, передавая e3App и job
    ...
    ' Обрабатываем каждый символ OOS
    For Each OOSNumber In OOSSymbols.Keys
        OOSSymbolId = OOSSymbols.Item(OOSNumber)
        
        e3App.PutInfo 0, "--- ОБРАБОТКА OOS" & OOSNumber & " ---"
        
        ' Получаем значение ОД E_Inom из текущего символа OOS
        OOSEInomValue = GetOOSAttributeEInom(e3App, job, OOSSymbolId) ' Передаем e3App и job
        e3App.PutInfo 0, "Получено значение ОД E_Inom из OOS" & OOSNumber & ": " & OOSEInomValue
        ...
        If qfDevices.Exists(OOSNumber) Then
            For Each key In qfDevices.Item(OOSNumber).Keys
                ...
                If Len("" & OOSEInomValue) > 0 Then
                    ...
                    Call WriteToQFDeviceTechDesc3(e3App, job, deviceId, OOSEInomValue) ' Передаем e3App и job
                End If

                If InStr(1, LCase(componentName), "автомат") > 0 Then
                    qfAutomatDeviceId = deviceId
                    e3App.PutInfo 0, "Найден QF Автомат" & OOSNumber & ": " & deviceName
                ElseIf InStr(1, LCase(componentName), "контакт") > 0 Then
                    ...
                    e3App.PutInfo 0, "Найден QF Контакт" & OOSNumber & ": " & deviceName
                End If
                
            Next
        Else
            e3App.PutInfo 0, "QF" & OOSNumber & " не найден"
        End If
        ...
        If kmDevices.Exists(OOSNumber) Then
            For Each key In kmDevices.Item(OOSNumber).Keys
                ...
                If InStr(1, LCase(componentName), "контактор") > 0 Then
                    ...
                    e3App.PutInfo 0, "Найден KM Контактор" & OOSNumber & ": " & deviceName
                    Exit For ' Берем только первый найденный контактор KM
                End If
            Next
        Else
            e3App.PutInfo 0, "KM" & OOSNumber & " не найден"
        End If
        ...
        If klDevices.Exists(OOSNumber) Then
            For Each key In klDevices.Item(OOSNumber).Keys
                ...
                klDeviceId = deviceId
                e3App.PutInfo 0, "Найден KL" & OOSNumber & ": " & deviceName
                Exit For ' Берем первое найденное KL устройство
            Next
        Else
            e3App.PutInfo 0, "KL" & OOSNumber & " не найден"
        End If
        ...
        If vDevices.Exists(OOSNumber) Then
            For Each key In vDevices.Item(OOSNumber).Keys
                ...
                vDeviceId = deviceId
                e3App.PutInfo 0, "Найден V" & OOSNumber & ": " & deviceName
                Exit For ' Берем первое найденное V устройство
            Next
        Else
            e3App.PutInfo 0, "V" & OOSNumber & " не найден"
        End If
        
        ' Записываем атрибуты в символ OOS (передаем e3App, job и новый vCompName)
    ...
    e3App.PutInfo 0, "=== ЗАВЕРШЕНИЕ ОБРАБОТКИ ВСЕХ OOS СИМВОЛОВ ==="
...
e3App_main.PutInfo 0, "=== СТАРТ СКРИПТА E3.SERIES OOS PROCESSOR ==="
