'*******************************************************************************
' Название скрипта: E3_ReplaceAllOOOWithSubcircuit
' Автор: E3.series VBScript Assistant
' Дата: 08.07.2025
' Описание: Скрипт предназначен для автоматической замены символов,
'           имена которых начинаются с "OOO" на соответствующие им фрагменты схемы.
...
'           "OOO" символа. Добавлены префиксы -tUZ и -tV для переименования.
'*******************************************************************************
Option Explicit
            ...
            If UCase(Left(symbolName, 3)) = "OOO" Then
                ...
                If attrValue <> "" And (attrValue >= "1" And attrValue <= "9") Or attrValue = "10" Then ' Расширил условие для "10"
                    ...
                    Select Case attrValue
                        Case "1": subcircuitPath = "W:\СПЕЦПРОЕКТЫ E3\БХК\ОДНОЛИН\фрагменты\Катя\1_tQF3P.e3p"
                        Case "2": subcircuitPath = "W:\СПЕЦПРОЕКТЫ E3\БХК\ОДНОЛИН\фрагменты\Катя\2_tQF3PtKM.e3p"
                        Case "3": subcircuitPath = "W:\СПЕЦПРОЕКТЫ E3\БХК\ОДНОЛИН\фрагменты\Катя\3_tАЗД3PtKM.e3p"
                        Case "4": subcircuitPath = "W:\СПЕЦПРОЕКТЫ E3\БХК\ОДНОЛИН\фрагменты\Катя\4_tАЗД2PtKM.e3p"
                        Case "5": subcircuitPath = "W:\СПЕЦПРОЕКТЫ E3\БХК\ОДНОЛИН\фрагменты\Катя\5_tQFd2PtKL.e3p"
                        Case "6": subcircuitPath = "W:\СПЕЦПРОЕКТЫ E3\БХК\ОДНОЛИН\фрагменты\Катя\6_tQF1P.e3p"
                        Case "7": subcircuitPath = "W:\СПЕЦПРОЕКТЫ E3\БХК\ОДНОЛИН\фрагменты\Катя\7_tQF3PtKM.e3p"
                        Case "8": subcircuitPath = "W:\СПЕЦПРОЕКТЫ E3\БХК\ОДНОЛИН\фрагменты\Катя\8_tАЗД2PtKL.e3p"
                        Case "9": subcircuitPath = "W:\СПЕЦПРОЕКТЫ E3\БХК\ОДНОЛИН\фрагменты\Катя\9_tQFd2.e3p"
                        Case "10": subcircuitPath = "W:\СПЕЦПРОЕКТЫ E3\БХК\ОДНОЛИН\фрагменты\Катя\10_tQF3PtKM.e3p"
                        Case Else: 
