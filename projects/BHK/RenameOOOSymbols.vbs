...
' === Главная процедура === Переименование символов OOO по позиции
Sub RenameOOOSymbolsByLocation()
    Dim e3App, job, symbol, sheet
    Dim allSymbolIds(), allSymbolCount
    Dim currentSymbolId, symbolName
    Dim s, i, j
    Dim oooCounter : oooCounter = 0 ' Счетчик для последовательного именования

    ' Переменные для хранения данных OOO символов для сортировки
    ' Каждый элемент в oooSymbolsToRename будет массивом:
    ' (SymbolID, SheetID, SheetName, Column, Row, X, Y, OriginalName)
    Dim oooSymbolsToRename()
    ReDim oooSymbolsToRename(0) ' Инициализация с фиктивным элементом, будет скорректировано

    Dim oooCountPlaced : oooCountPlaced = 0 ' Счетчик только для размещенных OOO символов
    ...
    e3App.PutInfo 0, "=== СТАРТ СКРИПТА: Переименование OOO символов по позиции ==="
    ...
    e3App.PutInfo 0, "Найдено " & allSymbolCount & " символов в проекте. Собираем данные и временно переименовываем OOO символы..."

    ' === Первый проход: Сбор данных о размещенных OOO символах и временное переименование ===
    For s = 1 To allSymbolCount
        ...
        If LCase(Left(symbolName, 3)) = "ooo" Then
            ...
            If sheetId > 0 Then ' Символ размещен на схеме
                ...
                ReDim Preserve oooSymbolsToRename(oooCountPlaced) ' Увеличиваем размер массива
                ...
                e3App.PutInfo 0, "  OOO символ найден на схеме: " & symbolName & " (ID: " & currentSymbolId & ") на листе: " & sheetName & " " & columnValue & rowValue & " (" & xPos & ", " & yPos & ")"
                
                ' Временно переименовываем символ, чтобы освободить имена OOO1, OOO2 и т.д.
                ...
                e3App.PutInfo 0, "  Временно переименован в 'OOOTMP" & Right(CStr(currentSymbolId), 6) & "'."
            Else
                e3App.PutInfo 0, "  OOO символ '" & symbolName & "' (ID: " & currentSymbolId & ") НЕ размещен на схеме. Он не будет переименован этим скриптом."
            End If
        End If
    Next

    If oooCountPlaced = 0 Then
        e3App.PutInfo 0, "Не найдено размещенных OOO символов на схеме для сортировки и переименования."
    ...
    e3App.PutInfo 0, "Найдено " & oooCountPlaced & " размещенных OOO символов. Сортировка по позиции (от меньшей к большей)..."

    ' === Сортировка oooSymbolsToRename (Bubble Sort) ===
    ' (SymbolID, SheetID, SheetName, Column, Row, X, Y, OriginalName)
    ...
    For i = 1 To oooCountPlaced - 1
        ...
        Next
    Next
    e3App.PutInfo 0, "Сортировка завершена. Приступаем к окончательному переименованию OOO символов."

    ' === Окончательное переименование OOO символов в отсортированном порядке ===
    For s = 1 To oooCountPlaced
        ...
        e3App.PutInfo 0, "  Символ '" & originalSymbolName & "' (ID: " & currentSymbolId & ") переименован в '" & newSymbolName & "' (на листе: " & oooSymbolsToRename(s)(2) & " " & oooSymbolsToRename(s)(3) & oooSymbolsToRename(s)(4) & ")."
    Next

    e3App.PutInfo 0, "=== ЗАВЕРШЕНИЕ СКРИПТА: Переименовано " & oooCounter & " OOO символов ==="
